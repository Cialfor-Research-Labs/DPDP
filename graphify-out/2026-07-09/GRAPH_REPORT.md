...
## Corpus Check
- 23 files · ~14,287 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 231 nodes · 325 edges · 19 communities (13 shown, 6 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 18 edges (avg confidence: 0.53)
...
1. `InterviewEngine` - 11 edges
2. `AuditLogManager` - 10 edges
3. `Obligations Database` - 10 edges
4. `LLMClient` - 9 edges
5. `ReportGenerator` - 9 edges
6. `RecommendationEngine` - 9 edges
7. `Question-by-Question Analysis` - 9 edges
8. `DeterministicChecker` - 8 edges
9. `TestReportGenerator` - 8 edges
10. `RetrievalMapper` - 7 edges

## Surprising Connections (you probably didn't know these)
- `TestReportGenerator` --uses--> `ReportGenerator`  [INFERRED]
  apps/api/tests/test_report_generator.py → apps/api/report_generator.py
- `TestReportGenerator` --uses--> `AuditLogManager`  [INFERRED]
  apps/api/tests/test_report_generator.py → apps/api/review_queue.py
- `TestDeterministicChecker` --uses--> `DeterministicChecker`  [INFERRED]
  apps/api/tests/test_deterministic_checker.py → apps/api/deterministic_checker.py
- `TestSemanticEvaluatorAndVerifier` --uses--> `DeterministicChecker`  [INFERRED]
  apps/api/tests/test_semantic_evaluator.py → apps/api/deterministic_checker.py
- `InterviewEngine` --uses--> `LLMClient`  [INFERRED]
  apps/api/interview_engine.py → apps/api/llm_client.py
...
Cohesion: 0.05
Nodes (36): 1. Graphify Output, 1. Graphify Output, 1. Graphify Output, 1. Graphify Output, 1. Graphify Output, 1. Graphify Output, 2. PDF Ground Truth, 2. PDF Ground Truth (+28 more)
...
Cohesion: 0.19
Nodes (7): Any, Calculates compliance scores.         - evaluation_results maps obligation_id ->, Generates remediation recommendations for non-compliant items.         Cites the, RecommendationEngine, ScoringEngine, TestReportGenerator, TestScoringAndRecommendationEngine
...
Cohesion: 0.23
Nodes (6): Any, Auto-derives ROPA categories and purposes from Section 5(1)/5(2) evidence., Builds the RACI matrix dynamically based on applicable obligation categories., Ranks recommendations by priority/severity:         - HIGH: SDF obligations, Chi, Assembles and saves a styled compliance HTML report using Jinja2 templates., ReportGenerator
...
Cohesion: 0.40
Nodes (5): 1. Graphify Output, 2. PDF Ground Truth, 3. Comparison, 4. Score, Q4 — Rights of a Data Principal

### Community 14 - "Q7 — Exemptions: What Exists and Which Sections They Attach To"
Cohesion: 0.40
Nodes (5): 1. Graphify Output, 2. PDF Ground Truth, 3. Comparison, 4. Score, Q2 — Conditions for Processing a Child's Personal Data
...
- **Why does `Question-by-Question Analysis` connect `Principal Rights and SDF Compliance` to `Q7 — Exemptions: What Exists and Which Sections They Attach To`, `Q7 — Exemptions: What Exists and Which Sections They Attach To`?**
...
- **Are the 2 inferred relationships involving `LLMClient` (e.g. with `InterviewEngine` and `TestInterviewEngine`) actually correct?**
  _`LLMClient` has 2 INFERRED edges - model-reasoned connections that need verification._
- **What connects `gating_params expected keys:         - role: "Data Fiduciary" or "Significant Da`, `Checks if an obligation applies to the current session parameters.         - If`, `Filter the full obligation list based on gating parameters.` to the rest of the system?**
  _89 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Consent and Data Subject Rights` be split into smaller, more focused modules?**
